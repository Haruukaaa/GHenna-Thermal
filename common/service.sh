...
for h in /proc/sys/kernel; do
   ...
   echo "0" "$h/panic_on_warn
   echo "0" "$h/panic_on_rcu_stall
   ...
   echo "0" > "$h/hung_task_timeout_secs"
   echo "0" > "$h/debug_locks"
   echo "0" > "$h/sched_schedstats"
   echo "0" > "$h/split_lock_mitigate"
   echo "32" > "$h/sched_nr_migrate"
   echo "0" > "$h/perf_event_paranoid"
   echo "1" > "$h/sched_child_runs_first"
   echo "4000000" > "$h/sched_latency_ns"
   echo "0" > "$h/sched_autogroup_enabled"
   echo "3" > "$h/perf_cpu_time_max_percent"
   echo "50000" > "$h/sched_migration_cost_ns"
   echo "1000000" > "$h/sched_min_granularity_ns"
   echo "0" > "$h/sched_min_task_util_for_colocation"
   echo "1500000" > "$h/sched_wakeup_granularity_ns"
done
